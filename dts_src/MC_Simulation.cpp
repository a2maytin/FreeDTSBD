...
bool MC_Simulation::CheckMesh(int step){
    ...
    // Note: Links only exist for membrane vertices, not DNA beads, so we don't need to filter here
    ...
    for (std::vector<links *>::const_iterator it = all_links.begin() ; it != all_links.end(); ++it){
        ...
        // DNA beads shouldn't have links, but we check bonds first to avoid calling GetVLinkList which can hang
        bool is_dna1 = !p_v1->GetBonds().empty();
        bool is_dna2 = !p_v2->GetBonds().empty();
        if (is_dna1 || is_dna2) continue;
